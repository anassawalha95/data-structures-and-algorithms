...
const countNumberOfChildren = (arr) => {
  return arr.reduce((acc, curr) => {
    return acc += (curr.children != null ? curr.children.length : 0)
  }, 0)
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((acc, curr, id) => { return acc += isPrime(curr) }, 0)
};
...
const extractChildren = (arr) => {
  return arr.reduce((acc, curr, id) => {
    ...
    if (curr.children != null) {
      ...
      ///c.log(curr.children.filter((val)=>  val.match(pattern)))
      acc = acc.concat(curr.children.filter((val) => val.match(pattern)))
...
describe('Testing challenge 11', () => {
  test('It should return an array containing the names of the children', () => {
    expect(extractChildren(characters)).toStrictEqual(['Sansa', 'Arya', 'Bran', 'Myrcella', 'Rhaegal', 'Margaery', 'Loras']);
    expect(extractChildren(characters).length).toStrictEqual(7);
  });
});
