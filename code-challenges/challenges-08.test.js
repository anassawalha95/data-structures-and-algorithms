...
const getStatName = (arr, minBaseStat) => {
  let statName = []
  arr.filter(val => (val.baseStat > minBaseStat ? statName.push(val.stat.name) : ""))
  return statName
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter((val) => val.children != null ? "" : val)
};
...
const evenOddNumericValues = (arr) => {
  ...
  let newArr = []
  arr.filter(val => {
    if (typeof val == "number" && val % 2 == 1) newArr.push("odd")
    if (typeof val == "number" && val % 2 == 0) newArr.push("even")
    return
  })
  return newArr
};
