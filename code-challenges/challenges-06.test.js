...
const sortByChildren = (charArray) => {
  charArray = charArray.sort((a, b) => {
    let firstNumberOfChlidren = a.children.length
    let secondNumberOfChlidren = b.children.length
    let firstHouseName = a.house
    let secondHouseNamen = b.house
    if (firstNumberOfChlidren > secondNumberOfChlidren) return 1;
    if (firstNumberOfChlidren < secondNumberOfChlidren) return -1;
    if (firstNumberOfChlidren == secondNumberOfChlidren) {
      if (firstHouseName > secondHouseNamen) return 1;
      if (firstHouseName > secondHouseNamen) return -1;
    }
    return 0
  });
...
const hasChildrenValues = (arr, character) => {
  ...
  for (let value of Object.values(arr)) {
    if (value.name.includes(character))
...
const totalCharacters = (arr) => {
  let count = 0
  for (let [key, value] of Object.entries(arr)) {
    if (value.name != null) count += 1
    if (value.spouse != null) count += 1
    if (value.children.length > 0) count += value.children.length
  }
  return count
};
...
const houseSurvivors = (arr) => {
  ...
  for (let [key, value] of Object.entries(arr)) {
    if (value.name != null) count += 1
    if (value.spouse != null && arr.includes(deceasedSpouses)) count += 1
    if (value.children.length > 0) count += value.children.length
    obj.house = value.house
    obj.members = count
    survivors.push(obj)
