...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(e => {
    result.push(e.slice(e.indexOf(" ", 3) + 1, e.length))
  });
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(e => {
    result.push(e.split(" ").slice(2).join(" "))
  });
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters.length - 1 > str.length)
    return ""
...
const totalSumCSV = (str) => {
  let total = 0;
  total = parseInt(str.split(",").reduce((a, b) => parseInt(a) + parseInt(b)))
  return total;
};
...
const extractVowels = (str) => {
  ...
  let updatedStr = []
  let extractVowels = []
  str.split("").filter((character) => {
    if (!vowels.includes(character)) {
      updatedStr.push(character)

    } else {
      extractVowels.push(character)
    }

  })

  updatedStr = [updatedStr.join(""), extractVowels.sort().join("")]
  return updatedStr
};
